...
${JSON.stringify(a,null,4)}`);let d=a;if(D.string(d.id)||D.number(d.id)){let p=d.id,v=ae.get(p);v&&v.reject(new Error("The received response has neither a result nor an error property."))}}function Te(a){if(a!=null)switch(N){case j.Verbose:return JSON.stringify(a,null,4);case j.Compact:return JSON.stringify(a);default:return}}function wi(a){if(!(N===j.Off||!$))if(de===re.Text){let d;(N===j.Verbose||N===j.Compact)&&a.params&&(d=`Params: ${Te(a.params)}

`),$.log(`Sending request '${a.method} - (${a.id})'.`,d)}else Ze("send-request",a)}function ki(a){if(!(N===j.Off||!$))if(de===re.Text){let d;(N===j.Verbose||N===j.Compact)&&(a.params?d=`Params: ${Te(a.params)}

`:d=`No parameters provided.

`),$.log(`Sending notification '${a.method}'.`,d)}else Ze("send-notification",a)}function Ut(a,d,p){if(!(N===j.Off||!$))if(de===re.Text){let v;(N===j.Verbose||N===j.Compact)&&(a.error&&a.error.data?v=`Error data: ${Te(a.error.data)}

`:a.result?v=`Result: ${Te(a.result)}

`:a.error===void 0&&(v=`No result returned.

`)),$.log(`Sending response '${d} - (${a.id})'. Processing request took ${Date.now()-p}ms`,v)}else Ze("send-response",a)}function xi(a){if(!(N===j.Off||!$))if(de===re.Text){let d;(N===j.Verbose||N===j.Compact)&&a.params&&(d=`Params: ${Te(a.params)}

`),$.log(`Received request '${a.method} - (${a.id})'.`,d)}else Ze("receive-request",a)}function xr(a){if(!(N===j.Off||!$||a.method===rr.type.method))if(de===re.Text){let d;(N===j.Verbose||N===j.Compact)&&(a.params?d=`Params: ${Te(a.params)}

`:d=`No parameters provided.

`),$.log(`Received notification '${a.method}'.`,d)}else Ze("receive-notification",a)}function zi(a,d){if(!(N===j.Off||!$))if(de===re.Text){let p;if((N===j.Verbose||N===j.Compact)&&(a.error&&a.error.data?p=`Error data: ${Te(a.error.data)}
